...
public class MyGame {
	...
	MyGame(){
		c = new Character("가렌", 100, 50, 20, 10, 10);
		
		items = new Item[10];
		items[0] = new Item("무한의대검", 0, 0, 10, 0,0);
		items[1] = new Item("가시갑옷", 0, 0, 0, 10,0);
		items[2] = new Item("쓸데없이 큰 지팡이", 0,0,0,0,20);
	}
	...
	void hunt(){
		...
		battle : while(true){
			...
			case 1:
				...
				break;
			case 2:
				System.out.println("1.마법1\t2.마법2");
				input = ScanUtil.nextInt();
				switch (input) {
				case 1: c.magic1(m);
				if(m.hp <= 0){
				...
				break;
			case 3:
				System.out.println(m.name +"은 재빠르게 도망갔습니다.");
				break battle;
			}
		}
	}
