package e.oop.restaurant;

public class Alba {
	//매서드 : 알바가 할 일 (보스가 알파한테 시킨 일)
	//파라미터 : 일을 하기 위해 필요한 것(보스가 알바에게 주는 것)
	//리턴타입 : 일을 하고 난 후 돌려주는 것(알바가 보스에게 주는 것)
	
	//주문받기 위해 필요한 것? 없음 parameter x
	//주문받은 후 보스에게 줄 것? 주문서 retrun o
	String[] order(){
		System.out.println("주문 하시겠습니까?");
		return new String[]{"짜장면","탕수육"};	
	}
	//서빙하기 위해 필요한 것? 음식 parameter o
	//서빙 후 보스에게 줄 것? 없음 return x
