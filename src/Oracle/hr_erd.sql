...
DROP TABLE HR.BUYPROD CASCADE CONSTRAINTS;
DROP TABLE HR.CART CASCADE CONSTRAINTS;
...
DROP TABLE HR.ZIPTB CASCADE CONSTRAINTS;
...
CREATE TABLE HR.BUYPROD
(
	-- 입고 일자
	BUY_DATE date NOT NULL,
	-- 상품 코드
	BUY_PROD varchar2(10 char) NOT NULL UNIQUE,
	-- 매입 수량
	BUY_QTY number(10,0) NOT NULL,
	-- 매입 단가
	BUY_COST number(10,0) NOT NULL,
	CONSTRAINT SYS_C007333 PRIMARY KEY (BUY_DATE, BUY_PROD)
);
...
(
	-- 회원 ID
	MEM_ID varchar2(15 char) NOT NULL UNIQUE,
	-- 주문 번호
	CART_NO char(13 char) NOT NULL,
	-- 상품 코드
	PROD_ID varchar2(10 char) NOT NULL UNIQUE,
	-- 수량
	CART_QTY number(8) NOT NULL,
	CONSTRAINT SYS_C007339 PRIMARY KEY (CART_NO, PROD_ID)
);
...
CREATE TABLE HR.ZIPTB
(
	-- 우편번호
	ZIPCODE char(7 char) NOT NULL,
	-- 특별시, 광역시, 도
	SIDO varchar2(2) NOT NULL,
	-- 시, 군, 구
	GUGUN varchar2(10) NOT NULL,
	-- 읍, 면, 동, 리, 건물명
	DONG varchar2(30) NOT NULL,
	-- 번지, 아파트 동, 호수
	BUNJI varchar2(10) NOT NULL,
	-- 자료 순서
...
COMMENT ON COLUMN HR.BUYPROD.BUY_DATE IS '입고 일자';
COMMENT ON COLUMN HR.BUYPROD.BUY_PROD IS '상품 코드';
COMMENT ON COLUMN HR.BUYPROD.BUY_QTY IS '매입 수량';
COMMENT ON COLUMN HR.BUYPROD.BUY_COST IS '매입 단가';
COMMENT ON COLUMN HR.CART.MEM_ID IS '회원 ID';
COMMENT ON COLUMN HR.CART.CART_NO IS '주문 번호';
COMMENT ON COLUMN HR.CART.PROD_ID IS '상품 코드';
COMMENT ON COLUMN HR.CART.CART_QTY IS '수량';
...
located. Foreign key to country_id column of the countries table.';
COMMENT ON COLUMN HR.ZIPTB.ZIPCODE IS '우편번호';
COMMENT ON COLUMN HR.ZIPTB.SIDO IS '특별시, 광역시, 도';
COMMENT ON COLUMN HR.ZIPTB.GUGUN IS '시, 군, 구';
COMMENT ON COLUMN HR.ZIPTB.DONG IS '읍, 면, 동, 리, 건물명';
COMMENT ON COLUMN HR.ZIPTB.BUNJI IS '번지, 아파트 동, 호수';
COMMENT ON COLUMN HR.ZIPTB.SEQ IS '자료 순서';
